...
public class AntColonyOptimization {
	...
	/**
	 * beta controls the distance priority. In general, the beta parameter should be greater than alpha for the best results.
	 */
	...
	/**
	 * antFactor tells us how many ants we�ll use per city
	 */
	...
	public double[][] generateRandomMatrix(int n) {
		...
		IntStream.range(0, n).forEach(i -> IntStream.range(0,  n).forEach(j -> randomMatrix[i][j] = Math.abs(random.nextInt(100) + 1)));
		return randomMatrix;
	}
	 ...
	 */
	public void startAndOptimization() {
	...
	public int[] solve() {
		setupAnts();
		clearTrails();
		IntStream.range(0, maxIteration).forEach(i -> {
			moveAnts();
			updateTrails();
			updateBest();
		});
	...
	private int selectNextCity(Ant ant) {
		...
		if (random.nextDouble() < randomFactor) {
			OptionalInt cityIndex = IntStream.range(0,  numberOfCities).filter(i -> i == t && !ant.visited(i)).findFirst();
